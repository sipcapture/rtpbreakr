  ...
  if (o.dump_wav)
	...
	if (strstr(find_stream_rtp_pt(rtp_stream->payload_type,1), "?") == NULL) {
		...
		if (strstr(codec, "711A") != NULL) {
		  	...
		  	snprintf(rtp_stream->command, sizeof(rtp_stream->command), "ffmpeg  -loglevel 0 -acodec pcm_alaw -f alaw -ar 8000 -i %s.%s -ar 8000 %s.mp3"
		  	...
		  	snprintf(rtp_stream->command, sizeof(rtp_stream->command), "ffmpeg  -loglevel 0 -acodec pcm_mulaw -f mulaw -ar 8000 -i %s.%s -ar 8000 %s.mp3"
										   ";ffmpeg  -loglevel 0 -i %s.mp3 -ac 1 -map 0:a -acodec pcm_s16le -f f32le - | gnuplot -p -e \"set terminal png transparent size 980,60 enhanced;set output '%s.mp3.png';unset key;unset tics;unset border;set lmargin 0;set rmargin 0;set tmargin 0.5;set bmargin 0.5; plot '<cat' binary filetype=bin format='%%int16' endian=little array=1:0 with lines;\" ",
										   namebody, codec, namebody, namebody, namebody);
        	  ...
        	  // contains some other codec, attempt conversion
		        snprintf(rtp_stream->command, sizeof(rtp_stream->command), "ffmpeg  -loglevel 0 -acodec %s -f %s -i %s.%s %s.mp3"
										   ";ffmpeg  -loglevel 0 -i %s.mp3 -ac 1 -map 0:a -acodec pcm_s16le -f f32le - | gnuplot -p -e \"set terminal png transparent size 980,60 enhanced;set output '%s.mp3.png';unset key;unset tics;unset border;set lmargin 0;set rmargin 0;set tmargin 0.5;set bmargin 0.5; plot '<cat' binary filetype=bin format='%%int16' endian=little array=1:0 with lines;\" ",
